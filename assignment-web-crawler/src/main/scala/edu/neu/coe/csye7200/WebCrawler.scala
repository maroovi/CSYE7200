...
object WebCrawler extends App {
  ...
  def wget(u: URL): Future[Seq[URL]] = {
    ...
    def getURLs(ns: Node): Seq[Try[URL]] =  {
      for (des <- ns.descendant; hrAtt <- des.attribute("href"))
        yield createURL(Try(new URL(u.getProtocol.concat(":"))).toOption, hrAtt.toString)
    } // TO BE IMPLEMENTED
    ...
    // 9 points.
    for (urlCont <- getURLContent(u)) yield getLinks(urlCont) match {
      case Success(usl) => usl
    } // TO BE IMPLEMENTED
  }
  ...
  def wget(us: Seq[URL]): Future[Seq[Either[Throwable, Seq[URL]]]] = {
    ...
    // TO BE IMPLEMENTED
    val us2fs = for (us2It <- us2) yield wget(us2It)
    val xUs2fs = for (us2f <- us2fs) yield MonadOps.sequence(us2f)
    Future.sequence(xUs2fs)
  }
