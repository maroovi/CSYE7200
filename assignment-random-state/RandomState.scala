...
trait RandomState[T] {
  ...
  def toStream: LazyList[T] =  LazyList.cons(get,next.toStream)// TO BE IMPLEMENTED
}
...
case class JavaRandomState[T](n: Long, g: Long => T) extends RandomState[T] {
  ...
  def next: RandomState[T] = JavaRandomState(random.nextLong(),g) // TO BE IMPLEMENTED
  // Hint: Think of the input and output.
  // 5 points
  def get: T = g(n) // TO BE IMPLEMENTED
  // Hint: This one need function composition.
  // 13 points
  def map[U](f: T => U): RandomState[U] = flatMap(_ => JavaRandomState(n, g andThen f))// TO BE IMPLEMENTED
}
...
object RandomState {
  ...
  // 4 points
  val longToDouble: Long => Double = _.toDouble // TO BE IMPLEMENTED
